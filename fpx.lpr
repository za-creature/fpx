...
uses
  {$ifdef win32}Windows, {$endif}Classes, Interfaces, Forms, uMain,
  uDirectoriesDialog, uCompilerModeDialog, uMemorySizesDialog, uLinkerDialog,
  uCompilerOptionsDialog, uGotoLineDialog, uRuntimeParametersDialog,
  uAboutDialog, uEditorOptionsDialog, uFindDialog, uPrimaryFileDialog,
  uEditorStyleDialog, uEditorOptionsController, uMessageBoxController,
  uAsciiTable, uTargetDialog, uProcessListWindow, uWatchListWindow,
  uAddWatchDialog, uDebugOptionsDialog, uBreakPointListWindow,
  uAddBreakPointDialog, uCallStackWindow, uRegistersWindow, uDisassemblyWindow,
  uHelpDialog, TurboPowerIPro, uHelpFilesDialog, Graphics,
  uDebugger, uAVLTree, uTagReader, uCompileDialog, uEvaluatorWindow,
  uBookmarkListWindow, uInternalEvaluatorDialog, uShowResultsDialog, Sysutils;
...
begin
  ...
  Application.CreateForm(TBookmarkListWindow, BookmarkListWindow);
  Application.CreateForm(TEvaluatorWindow, EvaluatorWindow);
  //dialogs
  ...
  Application.CreateForm(THelpFilesDialog, HelpFilesDialog);
  Application.CreateForm(TCompileDialog, CompileDialog);
  Application.CreateForm(TInternalEvaluatorDialog, InternalEvaluatorDialog);
  Application.CreateForm(TShowResultsDialog, ShowResultsDialog);
  //message box controller
  Application.CreateForm(TMessageBoxController, MessageBoxController);
  //register callbacks & set version
  AboutDialog.SetV(version);
