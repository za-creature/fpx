...
uses
  Classes, SysUtils,Graphics,Synedittypes,inifiles,Synedit,SynHighlighterPas,uEditorStyleWindow;
...
begin
  ...
  window_width:=conf.ReadInteger('window','width',640);
  window_height:=conf.ReadInteger('window','height',480);
  ...
  SetCurrentDir(conf.readString('ide','startup-dir',GetCurrentDir()));
...
begin
 ...
 conf.readString('ide','startup-dir',GetCurrentDir);
