...
uses
  ...
  uHelpFilesDialog, uAddBreakPointDialog, uAVLTree, uLinkerDialog,
  uCompileDialog;
       ...
       if PageController.PageCount>0 then
        begin
         ...
         for j:=0 to Editors.Count-1 do
          if ExtractFileName(GetEditor(j).ActiveFileName)=f then
           begin
            PageController.PageIndex:=j;
            found:=true;
            break;
           end;
           
{         if (not found)and
         (MessageBox('File not found','The requested file "'+f+'" could not be found. Would you like to locate the file manually?',MB_ICONQUESTION or MB_YESNOCANCEL)=MR_OK)
...
begin
     for i:=0 to Editors.Count-1 do
      if CygFormat(GetEditor(i).ActiveFileName)=filename then PageController.PageIndex:=i;
