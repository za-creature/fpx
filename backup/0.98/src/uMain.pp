...
uses
  ...
  uEditorStyleDialog, uRuntimeParametersDialog, uGotoLineDialog, uLinkerDialog,
  ...
  uCallStackWindow, uRegistersWindow, uDisassemblyWindow;
...
var br:integer;
begin
     br:=BreakPointListWindow.GetBreakPointId(IntToStr(GetActiveEditor().CaretY));
     if br=-1 then BreakPointListWindow.AddBreakPoint(IntToStr(GetActiveEditor().CaretY),true)
              else BreakPointListWindow.DeleteBreakPoint(br);
end;
...
//         new(p);
//         p^.cmdline:=EditorOptions.bin_directory+{$ifdef win32}'\'{$else}'/'{$endif}+'gdb'{$ifdef win32}+'.exe'{$endif};
{         p^.args:=exefile;
         p^.StartStamp:=FormatDateTime('h:nn:ss',Now());
         p^.hwnd:=0;
         p^.proc:=debugger.gdb.gdb;
         Processes.Add(p);}
...
begin
  ...
  if (debugger<>nil){and(ExtractFileName((Sender as TMyCustomEditor).activefilename)=debugger.CurrentFileName)}and(line=debugger.CurrentLineNumber)then
   begin
    Special:=true;
    FG:=ClBlack;
    BG:=$00FFCCCC;
