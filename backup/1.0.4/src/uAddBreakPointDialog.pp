...
uses
  ...
  Graphics, Dialogs, ExtCtrls, StdCtrls, Spin, Buttons, Synedit;
...
type
  ...
  public
    function Execute(gec:TGetEditorCallback;editorCount,activeEditor:integer):boolean;
...
function TAddBreakPointDialog.Execute(gec:TGetEditorCallback;editorCount:integer;activeEditor:integer):boolean;
var i:integer;
begin
     GetEditor:=gec;
     ...
     if editorCount<>0 then
      begin
       FileNameComboBox.ItemIndex:=activeEditor;
       LineNumber.Value:=GetEditor(activeEditor).CaretY;
      end;
     mr:=0;
     ShowModal();
     Result:=mr=1;
