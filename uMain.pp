...
procedure TMainForm.MainFormCreate(Sender: TObject);
  ...
  var i:integer;
      s:ansistring;
  begin
    //attempt to use USERPROFILE
    s:=GetEnvironmentVariable('UserProfile');
    if s<>'' then exit(s);
    //attempt to use HOMEDRIVE + HOMEPATH
    s:=GetEnvironmentVariable('HomeDrive');
    if s<>'' then
     if GetEnvironmentVariable('HomePath')<>'' then exit(s+GetEnvironmentVariable('HomePath'));
    //ATTEMPT TO USE APPDATA
...
begin
  ...
  //writeln(GetHomeDir());
  EditorOptions:=TEditorOptions.Create(GetHomeDir()+DirectorySeparator+'fpx.cfg');
...
begin
  if EditorsAvailable() then GetActiveEditor().Reload();
end;
