...
class procedure TAVLTree.Balance(var node:PTree);
begin
     FixHeight(node); 
...
class procedure TAVLTree.FixHeight(var node:PTree);
begin
     if node^.l^.h>node^.r^.h then node^.h:=node^.l^.h+1
                              else node^.h:=node^.r^.h+1; 
end; 

class procedure TAVLTree.RotLeft(var node:PTree);
var aux:PTree; 
begin
     aux:=node^.r;
     node^.r:=aux^.l; 
...
class procedure TAVLTree.RotRight(var node:PTree);
var aux:PTree; 
begin
     aux:=node^.l;
     node^.l:=aux^.r;
